...
class qzone{
	...
	public function cpdelll($uin,$id){
		...
		$data=$this->get_socket($url,$post,$this->cookie);
	...
	public function cpzhuanfa($con,$touin,$tid){
		...
		$json=$this->get_socket($url,$post,$this->cookie);
	...
	public function cpshuo($content,$richval='',$sname='',$lon='',$lat=''){
		...
		$result=$this->get_socket($url,$post,$this->cookie);
	...
	public function cpdel($cellid){
		...
		$json=$this->get_socket($url,$post,$this->cookie);
		//$json=$this->get_curl($url,$post,'http://m.qzone.com/infocenter?g_ut=3&g_f=6676',0,$this->cookie);
		$json=strstr($json,'{');
		$arr=json_decode($json,true);
		if(@array_key_exists('code',$arr) && $arr['code']==0){
	...
	public function getnew(){
		...
		if(function_exists("fsockopen")){
			$json=$this->get_socket($url);
		} else {
			$this->msg[]='您的空间不支持fsockopen函数，无法运行！';
			return false;
		}
		$arr=json_decode($json,true);
		if(@array_key_exists('code',$arr) && $arr['code']==0){
	...
	public function lzqd(){
		$url="http://share.music.qq.com/fcgi-bin/dmrp_activity/fcg_feedback_send_lottery.fcg?activeid=110&rnd=".time()."157&g_tk=".$this->gtk2."&uin=".$this->qq."&hostUin=0&format=jsonp&inCharset=GB2312&outCharset=gb2312&notice=0&platform=activity&jsonpCallback=checkInJsonCallback&needNewCode=1";
